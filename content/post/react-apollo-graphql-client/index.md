...
Once these packages are ready, we could now initialize our Apollo Client and wrap it as a React context provider under the **_main.jsx_** file.
...
We could write our GraphQL query command under the **_operations/user.js_** directory with the aid of imported `gql` function from `@apollo/client` package.
...
Let's navigate to the **_App.jsx_** file and work with the `useQuery` function.
...
Let's navigate to the **_App.jsx_** file and work with the `useMutation` function.
